...
int Delete(int pos)
{
    ...
    int x=-1;
    if(pos<1||pos>L)
        {cout<<"\n\nInvalid Positon We can't Delete ";
          return -1;
        }
     if(pos==1)
     {   p=head;
     ...
     else
     {
         ...
         x=p->data;
         p->prev->next=p->next;
            p->next->prev=p->prev;//also for last node
         delete p;
         L--;
     }
     return x;
...
int main()
{
    ...
    Insert(8,100);
    Display(head);
    cout<<"\nDeleted Element :"<<Delete(0);
    Display(head);
    cout<<"\nDeleted Element :"<<Delete(1);
    Display(head);
        cout<<"\nDeleted Element :"<<Delete(3);
    Display(head);
     cout<<"\nDeleted Element :"<<Delete(8);
    Display(head);
    Insert(7,319);
    Display(head);
}
